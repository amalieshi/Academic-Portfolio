...
NumControls = 2;  
%number of control experiments with no grating
dt = 1;  
%spacing between sampled time points [ms]
t_On = 0;  
%time stimulus turns on [ms]
t_Move = 500; 
%time stimulus begins moving [ms]
t_Off = 2500; 
%time stimulus turns off [ms]
NumAngles = size(spikes,1) -NumControls;  
%number of angles tested, equally spaced;   
%last 2 sets of recordings are controls
NumTimePoints = size(spikes,2);  
%number of time points; time was sampled every 1 ms
NumTrials = size(spikes,3);      
%number of trials performed at each angle

t_vect = t_On:dt:(NumTimePoints-1)*dt; 
%time vector for each trial

%PLOT RASTERS FOR ONE PARTICULAR ANGLE
figure(1)
plot(t_vect,spikes(2,:,20),'o')
xlabel('time (ms)');ylabel('spike=1, no spike=0')

%COMPUTE CV_isi
SpikeTimes_vect = dt*find(abs(spikes(ThisOrientation,TStartCount:...
    TEndCount,1)-1) < 0.00000001);
...
TrialSum_vect = sum(spikes(ThisOrientation,:,:),3);  
%adds up spike trains across trials
SmoothingWidth = 61; 
%smooths data over +/-[(this # -1)/2] bins
