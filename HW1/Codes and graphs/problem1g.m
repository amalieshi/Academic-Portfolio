...
%-------------------
% Define ODE as a function of membrane potential, V
dVdt = @(Vm1)  (10/(1+exp(-10*(Vm1-65)))*(Ei-Vm1))/C;
% Define span of time to evalute over (in miliseconds)
% Evaluates from 0 to 5 ms at intervals of 0.01 ms
t_span1 = [0:0.01:5];
...
% Run ODE23 to get V over time
[t1,Vm1] = ode23(@(t1,Vm1) dVdt(Vm1), t_span1, V0);
Vm1=Vm1.';
t1=t1.';

%---------------------------
% Define ODE as a function of membrane potential, V
dVdt = @(Vm2)  (10/(1+exp(-10*(Vm2-65))).*(Ei-Vm2)-Iapp)/C;
...
% Run ODE23 to get V over time
[t2,Vm2] = ode23(@(t2,Vm2) dVdt(Vm2), t_span2, V0);
Vm2=Vm2.';
t2=t2.';

%-------------------
% Define ODE as a function of membrane potential, V
dVdt = @(Vm3)  (10/(1+exp(-10*(Vm3-65))).*(Ei-Vm3))/C;
...
% Run ODE23 to get V over time
[t3,Vm3] = ode23(@(tm3,Vm3) dVdt(Vm3), t_span3, VX);
Vm3=Vm3.';
t3=t3.';

t=horzcat(t1,t2,t3);
V=horzcat(V1,V2,V3);
Vm=horzcat(Vm1,Vm2,Vm3);
...
plot(t,Vm,'--','DisplayName','Changing Conductance')
